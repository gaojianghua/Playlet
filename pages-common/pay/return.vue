<template>
	<view class="page">
		...
		<view class="content pt-10 d-flex a-center flex-column">
			...
			<u-image v-else-if="loading == 1" width="160rpx" height="160rpx" src="/static/img/common/lose.svg"></u-image>
			<u-image v-else width="160rpx" height="160rpx" src="/static/img/common/success.svg"></u-image>
			...
			<view v-if="loading != 0" @click="$tools.Navigate.reLaunch('/pages/tabbar/theater/index')" 
...
<script>
	import { getPayReturn } from '@/utils/request/api/get.js'
	export default {
		data() {
			return {
				value: 'page.付款中',
		...
		methods: {
			async getData() {
				let { code, data } = await getPayReturn(this.query)
				if (code == 200) {
					this.loading = 2
